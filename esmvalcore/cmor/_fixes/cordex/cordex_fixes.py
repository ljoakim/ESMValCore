...
class GridFix(Fix):
    ...
    def _fix_rotated_coords(self, cube, domain, domain_info, check=True):
    ...
    def _fix_geographical_coords(self, cube, domain, check=True):
...
class MPICSCREMO2009(GridFix):
    """Fixes for MPI-CSC-REMO2009."""
    ...
    def fix_metadata(self, cubes):
        ...
        domain_info = _get_domain_info(data_domain)
        for cube in cubes:
            self._fix_rotated_coords(
                cube, domain, domain_info, check=False
            )
            self._fix_geographical_coords(cube, domain, check=False)
        return cubes
